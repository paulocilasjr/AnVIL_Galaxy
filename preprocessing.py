...
# subsets to selected tissues, saves the processed matrix and metadata, generates images from the matrix,
# creates ludwig_input.csv with image_path and label, and zips the JPG images for Ludwig input.
...
# Bucket for uploads (replace with your actual bucket name, e.g., 'gs://fc-secure-your-bucket/')
BUCKET = 'gs://your-bucket-name/'

def resolve_drs(drs_uri):
    """Resolve DRS URI to a signed URL."""
    return drs.access(drs_uri)
...
def load_annotations(signed_url):
    ...
    return pd.read_csv(signed_url, sep='\t')
...
def generate_images(matrix_path, jpg_dir, normalization):
    # Simulate argparse for main function
    class Args:
        input = matrix_path
        sep = ','
        normalize = normalization
        tensor_output = 'tensor.npy'
        jpg_dir = jpg_dir
        no_save_tensor = False

    args = Args()
    df = load_matrix(args.input, sep=args.sep)
    tensor, image_size = convert_matrix_to_images(df, normalization=args.normalize)
    if not args.no_save_tensor:
        np.save(args.tensor_output, tensor)
        print(f"Saved tensor: {args.tensor_output} with shape {tensor.shape}")
    save_images_as_jpg(tensor, args.jpg_dir, sample_names=df.index)
    print(f"Saved {tensor.shape[0]} JPEG images to directory: {args.jpg_dir}")
...
def full_pipeline():
    ...
    # reads_signed_url = resolve_drs(READS_DRS)  # Uncomment if using reads
    ...
    # Generate images
    generate_images('expression_matrix.csv', JPG_DIR, NORMALIZATION_METHOD)

    # Upload images to bucket
    upload_to_bucket(JPG_DIR, BUCKET + JPG_DIR + '/')
